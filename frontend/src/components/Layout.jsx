...
import {
  ...
  Bot, Mic, Settings, Sparkles, ChevronRight, LogOut, Bug,
} from 'lucide-react'
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      <aside className="w-60 shrink-0 flex flex-col relative"
        style={{ background: 'rgba(5,5,10,0.85)', backdropFilter: 'blur(18px)', borderRight: '1px solid rgba(255,255,255,0.07)', zIndex: 20 }}>

        {/* Logo */}
        <div className="px-5 pt-6 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 flex items-center justify-center"
              style={{ border: '1px solid rgba(255,255,255,0.18)', transform: 'rotate(45deg)', background: 'rgba(255,255,255,0.04)', flexShrink: 0 }}>
              <span className="text-xs font-bold text-white"
                style={{ transform: 'rotate(-45deg)', fontFamily: 'var(--font-serif)', fontStyle: 'italic' }}>A</span>
            </div>
            <div>
              <h1 className="text-base text-white leading-tight"
                style={{ fontFamily: 'var(--font-serif)', fontStyle: 'italic', fontWeight: 700 }}>Atlas</h1>
          ...
          {NAV.map(({ to, icon: Icon, label, mono }) => (
            <NavLink key={to} to={to}
              ...
              style={({ isActive }) => isActive
                ? { background: 'rgba(99,102,241,0.13)', boxShadow: 'inset 0 0 0 1px rgba(99,102,241,0.22)' }
                : {}}>
              {({ isActive }) => (
                <>
                  {isActive && (
                  ...
                  <Icon size={15} className={`shrink-0 transition-colors ${isActive ? 'text-indigo-400' : 'text-slate-600 group-hover:text-slate-400'}`} />
